...
export async function ragSearch({ query, pdfIds, topK = 5 }) {
  ...
  for (const pdfId of pdfIds) {
    ...
    const docs = await collection.query({ queryEmbeddings: [queryVec], nResults: topK });
  ...
  const context = results.map(r => `Page ${r.page}: ${r.text}`).join('\n---\n');
  ...
  const citations = results.map(r => ({ page: r.page, quote: r.text.split('. ').slice(0,2).join('. ') }));
