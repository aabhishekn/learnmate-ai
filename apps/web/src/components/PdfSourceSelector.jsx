    ...
    <div className="bg-white rounded shadow p-4 mb-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
        <div className="flex gap-2">
          <button
            className={`px-4 py-2 rounded ${!selectedId ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700'} transition`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {pdfs.map(pdf => (
          <div
            key={pdf.id}
            className={`border rounded p-3 flex items-center justify-between cursor-pointer ${selectedId === pdf.id ? 'border-indigo-500 bg-indigo-50' : 'hover:bg-gray-50'}`}
